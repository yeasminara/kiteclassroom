...
<section class="wrapper">
      <div class="row">
            <div class="col-lg-12">
                <h3 class="page-header"><i class="fa fa fa-bars"></i> Home Work Assign</h3>
                <ol class="breadcrumb">
   ...
   <div class="content-box-large">
                ...
                <form action="<?=base_url().$this->uri->uri_string()?>" method="post" enctype="multipart/form-data">
  					<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered">
                        ...
                        <td><input type="checkbox" name="selected_student[]" value="<?php echo $result['uid'];?>" class="selected_user form-control" <?php if($check_permission['is_complete']==0 && $check_permission['is_evaluate']==0) {?> checked="checked"  disabled="disabled"<?php }?>></td>
                        
                        <td><?=$result['roll_no']?></td>
                        <td><?=$result['first_name'].' '.$result['last_name']?></td>
                        <td><?=$result['section_name']?></td>
                        <td><?=date('d-m-Y',strtotime($result['create_date']))?></td>
                        <td><?php if($check_permission['is_complete']==0 && $check_permission['is_evaluate']==0){ echo 'Assigned'; } elseif($check_permission['is_evaluate']==1 && $check_permission['is_complete']==1){ echo 'Evaluated';  }else{ echo '';}?></td>
                    ...
                    <input type="submit" class="btn btn-primary" name="submit" value="Assign Home Work"/>
