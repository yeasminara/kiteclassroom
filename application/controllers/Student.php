...
class Student extends CI_Controller {
	...
	public function index($limit='0')
	{
		...
		// fetching user list
		$data['result']=$this->user_model->student_list($limit);
	/*	$this->load->view('header',$data);
		$this->load->view('user_list',$data);
		$this->load->view('footer',$data);*/
		
		$data['middle'] = $this->load->view('student/user_list',$data, true);
	...
	public function new_user()
	{
	    ...
	    $result_role = $query_role->row_array();
		 ...
		 if($result_role['role_type']=='school_teacher' || $result_role['role_type']=='school_admin'){
		  ...
		  $data['user_role'] = $result_role['role_type'];
	   ...
	   /* $this->load->view('header',$data);
		...
		$data['middle'] = $this->load->view('student/new_user',$data, true);
        $this->load->view('template', $data);
	}
          ...
          if ($this->form_validation->run() == FALSE)
                {
					...
					redirect('student/new_user/');
                }
                else
                {
					if($this->user_model->insert_student()){
		...
		//echo $this->db->last_query();
		 ...
		 //$this->load->view('header',$data);
			if($results['role_type'] == 'administrator' || $results['role_type'] == 'school_admin'){
				...
				$data['middle'] =$this->load->view('student/edit_user',$data, true);
			}else{
           ...
           if ($this->form_validation->run() == FALSE)
                {
					echo 'test1';
                     $this->session->set_flashdata('message', "<div class='alert alert-danger'>".validation_errors()." </div>");
					redirect('student/update_user/'.$uid);
                }
                else
                {
					
					if($this->user_model->update_student($uid)){
						//echo 'test255';
						//echo $this->db->last_query();
					...
					//redirect(''.$_POST['url'].'');
					redirect('student/edit_user/'.$uid);
                }       
...
=================Student Promotion list=============================================*/	
	...
	public function student_promotion_list(){
		...
		$this->load->view('student_promotion_list',$data);
