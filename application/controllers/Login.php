...
class Login extends CI_Controller {
		...
		public function registration()
	{
		...
		// fetching group list
		$data['group_list']=$this->db->where('group_name !=','Administartor')->get('savsoft_group')->result_array();
	...
	public function verifylogin(){
		...
		if($this->user_model->login($username,$password)){
			...
			$user['role_type']=$result_role['role_type'];
			$user['role_title']=$result_role['title'];
			// creating login cookie
			$this->session->set_userdata('logged_in', $user);
			//$this->session->set_userdata('user_group', $user['gid']);
			...
			if($result_role['title']=='Student'){
				...
				redirect('parents');
			}elseif($result_role['title']=='School Teacher'){
				redirect('teacher');
			}else{
				redirect('dashboard');
			}
	...
	function show_messages(){
	 ...
	 $this->session->set_flashdata('message', 'Don&prime;t have permission. Please ask adminstartion for permission');
		//redirect('/login/');
		...
		$data['middle'] = $this->load->view('message',$data, true);
